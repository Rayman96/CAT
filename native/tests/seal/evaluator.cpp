...
#include "seal/randomgen.h"
        ...
        }
        {
            ...
            for (int round = 0; round < 100; round++)
            {
                ...
                for (size_t i = 0; i < slot_size; i++)
                {
                    auto tmp = abs(expected[i].real() - output[i].real());
                    if (tmp > 0.5){
                        printf("i:%d, output:%f, expected:%f\n", i, output[i].real(), expected[i].real());
                    }
                    ASSERT_TRUE(tmp < 0.5);
                }
            }
    ...
    TEST(EvaluatorTest, CKKSEncryptMultiplyRelinDecrypt)
    {
        ...
        }
        {
            ...
            size_t slot_size = 2048;
            parms.set_poly_modulus_degree(slot_size * 2);
            parms.set_coeff_modulus(CoeffModulus::Create(slot_size * 2, { 60, 30, 30, 60 }));
    ...
    TEST(EvaluatorTest, CKKSEncryptSquareRelinDecrypt)
    {
        ...
        }
        {
            ...
            size_t slot_size = 2048;
            parms.set_poly_modulus_degree(slot_size * 2);
            parms.set_coeff_modulus(CoeffModulus::Create(2048, { 60, 30, 30, 60 }));
    ...
    TEST(EvaluatorTest, CKKSEncryptMultiplyRelinRescaleDecrypt)
    {
        EncryptionParameters parms(scheme_type::ckks);
        {
            ...
            size_t slot_size = 4096;
            parms.set_poly_modulus_degree(slot_size * 2);
            parms.set_coeff_modulus(CoeffModulus::Create(slot_size * 2, { 60, 60, 60, 60, 60, 60 }));
        ...
        }
        {
            ...
            parms.set_poly_modulus_degree(slot_size * 8);
            parms.set_coeff_modulus(CoeffModulus::Create(slot_size * 8, { 60, 60, 60, 60, 60 }));
    ...
    //         parms.set_poly_modulus_degree(4096);
    //         parms.set_plain_modulus(plain_modulus);
    //         parms.set_coeff_modulus(CoeffModulus::Create(4096, { 40, 40 }));
    ...
    TEST(EvaluatorTest, BFVEncryptMultiplyPlainNTTDecrypt)
    {
        ...
        parms.set_plain_modulus(plain_modulus);
        parms.set_coeff_modulus(CoeffModulus::Create(2048, { 40, 40 }));
