...
namespace seal
{
    ...
    std::pair<int, int> split_into_closest_powers_of_two(int number) {

        int a = 1, b = 1; 
        double minDiff = number;
        
        for (int i=1; i<=log2(number); i++) {
            int m = pow(2, i);
            if (abs(number - m*m) < minDiff) {
            minDiff = abs(number - m*m);
            a = m;
            b = number/m; 
            }
        }
    ...
    void get_matrix_n1(
        int nx, int ny, uint64_t root, uint64_t modulu, uint64_t coeff_modulus_size, uint64_t *matrix)
    {
        dim3 block(16, 16);
        dim3 grid((nx - 1) / block.x + 1, (ny - 1) / block.y + 1);
        fillTablePsi128_root<<<grid, block>>>(root, modulu, matrix, nx, ny);
    }

    void get_matrix_n12(
        int n1, int n2, uint64_t root, uint64_t modulu, uint64_t coeff_modulus_size, uint64_t *matrix)
    {
        dim3 block(16, 16);
        dim3 grid((n2 - 1) / block.x + 1, (n1 - 1) / block.y + 1);
        fillTablePsi128_root_n12<<<grid, block>>>(root, modulu, matrix, n1, n2);
    }
        ...
        int nx, int ny, uint64_t root, uint64_t modulu, uint64_t coeff_modulus_size, uint64_t *matrix)
    {
        dim3 block(16, 16);
        dim3 grid((nx - 1) / block.x + 1, (ny - 1) / block.y + 1);
        fillTablePsi128_root_n2<<<grid, block>>>(root, modulu, matrix, nx, ny);
    }
    ...
    SEALContext::ContextData SEALContext::validate(EncryptionParameters parms)
    {
        ...
        try
        {
            ...
            // 创建d_root_powers_
            size_t c_vec_size = mul_safe(poly_modulus_degree, coeff_modulus.size());
            uint64_t h_root_powers[c_vec_size];
            int h_bit_count[coeff_modulus.size()];
            uint64_t h_roots[coeff_modulus.size()];

            for (int i = 0; i < coeff_modulus.size(); i++)
            {
                auto wrap_root_powers = context_data.small_ntt_tables_.get()[i].get_from_root_powers();
                h_bit_count[i] = context_data.small_ntt_tables_.get()[i].coeff_count_power();
                h_roots[i] = context_data.small_ntt_tables_.get()[i].get_root();
                for (int j = 0; j < poly_modulus_degree; j++)
                {
                    h_root_powers[i * poly_modulus_degree + j] = wrap_root_powers[j].operand;
                }
            }
            ...
            checkCudaErrors(cudaMemcpy(
                context_data.d_bit_count_, h_bit_count, coeff_modulus.size() * sizeof(int), cudaMemcpyHostToDevice));
            checkCudaErrors(cudaMemcpy(
                context_data.d_roots_, h_roots, coeff_modulus.size() * sizeof(uint64_t), cudaMemcpyHostToDevice));
           

            // int n1 = 128, n2 = 64;
#if NTT_VERSION == 3
            std::pair<int, int> result = split_into_closest_powers_of_two(poly_modulus_degree);
            int n1 = result.first, n2 = result.second;
            context_data.n1_ = n1;
            context_data.n2_ = n2;
            checkCudaErrors(cudaMalloc((void **)&context_data.d_root_matrix_n1_, n1 * n1 * coeff_modulus.size() * sizeof(uint64_t)));
            checkCudaErrors(cudaMalloc((void **)&context_data.d_root_matrix_n2_, n2 * n2 * coeff_modulus.size() * sizeof(uint64_t)));
            checkCudaErrors(cudaMalloc((void **)&context_data.d_root_matrix_n12_, n1 * n2 * coeff_modulus.size() * sizeof(uint64_t)));
            for(int i = 0; i < coeff_modulus.size(); i++) {
                uint64_t modulu = coeff_modulus[i].value();
                uint64_t root_n12 = context_data.small_ntt_tables_.get()[i].get_root();
                uint64_t root_n1 = power_m(root_n12, poly_modulus_degree / n1, coeff_modulus[i]);
                uint64_t root_n2 = power_m(root_n12, poly_modulus_degree / n2, coeff_modulus[i]);

                get_matrix_n1(n1, n1, root_n1, modulu, coeff_modulus_size, context_data.d_root_matrix_n1_ + i * n1 * n1);
                get_matrix_n2(n2, n2, root_n2, modulu, coeff_modulus_size, context_data.d_root_matrix_n2_ + i * n2 * n2);
                get_matrix_n12(n1, n2, root_n12, modulu, coeff_modulus_size, context_data.d_root_matrix_n12_ + i * n1 * n2);
            }
#endif 
            // 创建inv_table
            ...
            for (int i = 0; i < coeff_modulus.size(); i++)
            {
                fillTablePsi128<<<(poly_modulus_degree + 1023) / 1024, 1024>>>(
                    context_data.small_ntt_tables_.get()[i].get_inv_root(), coeff_modulus[i].value(),
                    context_data.d_inv_root_powers_ + i * poly_modulus_degree,
                    context_data.small_ntt_tables_.get()[i].coeff_count_power());
            }
        }
        catch (const invalid_argument &)
        {
...
// palin NTT table
auto plain_modulu = parms.plain_modulus();
            ...
            context_data.n1_ = n1;
            context_data.n2_ = n2;
            checkCudaErrors(cudaMalloc((void **)&context_data.d_plain_root_matrix_n1_, n1 * n1  * sizeof(uint64_t)));
            checkCudaErrors(cudaMalloc((void **)&context_data.d_plain_root_matrix_n2_, n2 * n2 * sizeof(uint64_t)));
            checkCudaErrors(cudaMalloc((void **)&context_data.d_plain_root_matrix_n12_, n1 * n2 * sizeof(uint64_t)));

            uint64_t root_n12 = context_data.plain_ntt_tables_.get()[0].get_root();
            uint64_t root_n1 = power_m(root_n12, poly_modulus_degree / n1, plain_modulu);
            uint64_t root_n2 = power_m(root_n12, poly_modulus_degree / n2, plain_modulu);

            get_matrix_n1(n1, n1, root_n1, plain_modulu.value(), coeff_modulus_size, context_data.d_plain_root_matrix_n1_);
            get_matrix_n2(n2, n2, root_n2, plain_modulu.value(), coeff_modulus_size, context_data.d_plain_root_matrix_n2_);
            get_matrix_n12(n1, n2, root_n12, plain_modulu.value(), coeff_modulus_size, context_data.d_plain_root_matrix_n12_);
#endif
...
// plain INTT table
            ...
            fillTablePsi128<<<(poly_modulus_degree + 1023) / 1024, 1024>>>(
                ...
                context_data.plain_ntt_tables_.get()[0].coeff_count_power());
            ...
            uint64_t *host_coeff_div_plain_modulus_operand = new uint64_t[coeff_modulus_size];
            uint64_t *host_coeff_div_plain_modulus_quoient = new uint64_t[coeff_modulus_size];
            ...
            for (size_t i = 0; i < coeff_modulus_size; i++)
            {
                ...
                host_coeff_div_plain_modulus_operand[i] = context_data.coeff_div_plain_modulus_[i].operand;
            ...
            checkCudaErrors(cudaMemcpy(
                context_data.d_coeff_div_plain_modulus_quoitent_, host_coeff_div_plain_modulus_quoient, coeff_modulus_size * sizeof(uint64_t),
                cudaMemcpyHostToDevice));
    ...
    void SEALContext::ensure_ntt_size(size_t current_size) const
    {
        if (current_size > ntt_temp_size_)
        {
            checkCudaErrors(cudaMalloc((void **)&ntt_temp_, current_size * sizeof(uint64_t)));
            ntt_temp_size_ = current_size;
        }
    }

    uint64_t SEALContext::estimate_size()
    {
    ...
    // 生成多个stream
        ...
        if (parms.coeff_modulus().size() > 8)
        {
            num_streams_ = 8;
        }
        else
        {
            num_streams_ = parms.coeff_modulus().size();
        }
