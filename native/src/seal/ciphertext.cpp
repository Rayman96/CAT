...
namespace seal
{
    Ciphertext &Ciphertext::operator=(const Ciphertext &assign)
    {
        ...
        if (assign.data_.cbegin() != nullptr){
    ...
    void Ciphertext::resize(const SEALContext &context, parms_id_type parms_id, size_t size)
    {
        ...
        // printf("poly_modulus: %d\n", parms.coeff_modulus().size());
        resize_internal(size, parms.poly_modulus_degree(), parms.coeff_modulus().size());
    }

    void Ciphertext::resize_pure_gpu(const SEALContext &context, parms_id_type parms_id, size_t size)
    {
        // Verify parameters
        if (!context.parameters_set())
        {
        ...
        size_t new_data_size = mul_safe(size_, poly_modulus_degree_, coeff_modulus_size_);
        data_.resize_only_param(new_data_size);

        resize_gpu(size, poly_modulus_degree_, coeff_modulus_size_);
    }
