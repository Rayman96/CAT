    ...
    */
    ...
    public:
        ...
        */
        ...
        inline void set_d_coeff_modulus(const std::vector<Modulus> &coeff_modulus)
        {
            size_t coeff_modulus_size = coeff_modulus.size();
            uint64_t coeff_modulus_value[coeff_modulus_size];
            uint64_t coeff_modulus_ratio_0[coeff_modulus_size];
            uint64_t coeff_modulus_ratio_1[coeff_modulus_size];
            for (size_t i = 0; i < coeff_modulus_size; i++)
            {
                coeff_modulus_value[i] = coeff_modulus[i].value();
                coeff_modulus_ratio_0[i] = coeff_modulus[i].const_ratio().data()[0];
                coeff_modulus_ratio_1[i] = coeff_modulus[i].const_ratio().data()[1];
            }

            checkCudaErrors(cudaMalloc((void **)&d_coeff_modulus_value_, coeff_modulus_size * sizeof(std::uint64_t)));
            checkCudaErrors(cudaMalloc((void **)&d_coeff_modulus_ratio_0_, coeff_modulus_size * sizeof(std::uint64_t)));
            checkCudaErrors(cudaMalloc((void **)&d_coeff_modulus_ratio_1_, coeff_modulus_size * sizeof(std::uint64_t)));
    

            checkCudaErrors(cudaMemcpy(
                d_coeff_modulus_value_, coeff_modulus_value, coeff_modulus_size * sizeof(std::uint64_t),
                cudaMemcpyHostToDevice));
            checkCudaErrors(cudaMemcpy(
                d_coeff_modulus_ratio_0_, coeff_modulus_ratio_0, coeff_modulus_size * sizeof(std::uint64_t),
                cudaMemcpyHostToDevice));
            checkCudaErrors(cudaMemcpy(
                d_coeff_modulus_ratio_1_, coeff_modulus_ratio_1, coeff_modulus_size * sizeof(std::uint64_t),
                cudaMemcpyHostToDevice));
        }
        ...
        */
        inline void set_coeff_modulus(const std::vector<Modulus> &coeff_modulus)
        {
            ...
            set_d_coeff_modulus(coeff_modulus);
        ...
        */
        ...
        SEAL_NODISCARD inline std::uint64_t *d_coeff_modulus_value() const noexcept
        {
            return d_coeff_modulus_value_;
        }

        SEAL_NODISCARD inline std::uint64_t *d_coeff_modulus_ratio_0() const noexcept
        {
            return d_coeff_modulus_ratio_0_;
        }

        SEAL_NODISCARD inline std::uint64_t *d_coeff_modulus_ratio_1() const noexcept
        {
            return d_coeff_modulus_ratio_1_;
        }
    ...
    private:
        ...
        */
        ...
        std::uint64_t *d_coeff_modulus_value_ = nullptr;

        std::uint64_t *d_coeff_modulus_ratio_0_ = nullptr;

        std::uint64_t *d_coeff_modulus_ratio_1_ = nullptr;
