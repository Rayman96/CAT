...
namespace seal
{
    ...
    */
    ...
    private:
        ...
        mutable uint64_t *d_t_target_ = nullptr; //copy了一份
        mutable uint64_t *d_t_operand_ = nullptr;
        mutable uint64_t *d_t_poly_lazy_ = nullptr;
        mutable uint64_t *d_t_poly_prod_iter_ = nullptr;

        mutable size_t d_temp_sk_size=0;
        mutable size_t d_t_target_size=0;
        mutable size_t d_t_operand_size=0;
        mutable size_t d_t_poly_lazy_size=0;
        mutable size_t d_t_poly_prod_iter_size=0;

// bfv_multiply使用，后续可以和上边的共享
        mutable uint64_t *d_encrypted1_q = nullptr;
        mutable uint64_t *d_encrypted2_q = nullptr;
        mutable uint64_t *d_encrypted1_Bsk = nullptr;
        mutable uint64_t *d_encrypted2_Bsk = nullptr;
        mutable uint64_t *d_temp_dest_q = nullptr;
        mutable uint64_t *d_temp_dest_Bsk = nullptr;
        mutable uint64_t *d_temp_q_Bsk=nullptr;
        mutable uint64_t *d_temp_Bsk = nullptr;
        mutable uint64_t *d_temp_bfv_multiply = nullptr;

        mutable size_t d_encrypted1_q_size = 0;
        mutable size_t d_encrypted2_q_size = 0;
        mutable size_t d_encrypted1_Bsk_size = 0;
        mutable size_t d_encrypted2_Bsk_size = 0;
        mutable size_t d_temp_dest_q_size = 0;
        mutable size_t d_temp_dest_Bsk_size = 0;
        mutable size_t d_temp_q_Bsk_size=0;
        mutable size_t d_temp_Bsk_size = 0;
        mutable size_t d_temp_bfv_multiply_size = 0;
