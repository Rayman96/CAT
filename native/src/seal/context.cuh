    ...
    */
    ...
    public:
        ...
        */
        ...
        public:
            ...
            */
            ...
            SEAL_NODISCARD inline std::uint64_t *d_total_coeff_modulus() const noexcept
            {
                return d_total_coeff_modulus_;
            }
            ...
            */
            ...
            SEAL_NODISCARD inline std::uint64_t *d_root_powers() const noexcept
            {
                return d_root_powers_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_root_matrix() const noexcept
            {
                return d_root_matrix_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_root_matrix_n1() const noexcept
            {
                return d_root_matrix_n1_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_root_matrix_n2() const noexcept
            {
                return d_root_matrix_n2_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_root_matrix_n12() const noexcept
            {
                return d_root_matrix_n12_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_plain_root_matrix_n1() const noexcept
            {
                return d_plain_root_matrix_n1_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_plain_root_matrix_n2() const noexcept
            {
                return d_plain_root_matrix_n2_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_plain_root_matrix_n12() const noexcept
            {
                return d_plain_root_matrix_n12_;
            }


            SEAL_NODISCARD inline std::uint64_t *d_inv_root_powers() const noexcept
            {
                return d_inv_root_powers_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_plain_inv_root_powers() const noexcept
            {
                return d_plain_inv_root_powers_;
            }

            SEAL_NODISCARD inline std::pair<int, int> split_degree() const noexcept
            {
                return std::make_pair(n1_, n2_);
            }

            SEAL_NODISCARD inline int *d_bit_count() const noexcept
            {
                return d_bit_count_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_roots() const noexcept
            {
                return d_roots_;
            }
            /**
            ...
            */
            ...
            SEAL_NODISCARD inline std::uint64_t *d_coeff_div_plain_modulus_operand() const noexcept
            {
                return d_coeff_div_plain_modulus_operand_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_coeff_div_plain_modulus_quoitent() const noexcept
            {
                return d_coeff_div_plain_modulus_quoitent_;
            }
            ...
            */
            ...
            SEAL_NODISCARD inline std::uint64_t *d_upper_half_threshold() const noexcept
            {
                return d_upper_half_threshold_;
            }

            SEAL_NODISCARD inline std::uint64_t *d_plain_upper_half_increment() const noexcept
            {
                return d_plain_upper_half_increment_;
            }
        ...
        private:
            ...
            uint64_t *d_root_powers_ = nullptr;

            uint64_t *d_root_matrix_ = nullptr; // modulu_size * coeff_count * coeff_count

            uint64_t *d_inv_root_powers_ = nullptr;

            uint64_t *d_total_coeff_modulus_ = nullptr;

            uint64_t *d_upper_half_threshold_ = nullptr;

            uint64_t *d_plain_upper_half_increment_ = nullptr;

            uint64_t *d_coeff_div_plain_modulus_operand_ = nullptr;
            
            uint64_t *d_coeff_div_plain_modulus_quoitent_ = nullptr;

            uint64_t *d_root_matrix_n1_ = nullptr;

            uint64_t *d_root_matrix_n2_ = nullptr;

            uint64_t *d_root_matrix_n12_ = nullptr;

            uint64_t *d_plain_root_matrix_n1_ = nullptr;

            uint64_t *d_plain_root_matrix_n2_ = nullptr;

            uint64_t *d_plain_root_matrix_n12_ = nullptr;

            uint64_t *d_plain_inv_root_powers_ = nullptr;

            size_t n1_ = 0;

            size_t n2_ = 0;

            uint64_t *d_roots_ = nullptr;

            int *d_bit_count_ = nullptr;
        };
        ...
        */
        ...
        SEAL_NODISCARD inline std::uint64_t *ntt_temp() const noexcept
        {
            return ntt_temp_;
        }

        SEAL_NODISCARD inline size_t &ntt_temp_size() const noexcept
        {
            return ntt_temp_size_;
        }

        SEAL_NODISCARD inline cudaStream_t *stream_context() const noexcept
        {
            return streams_;
        }

        SEAL_NODISCARD inline int num_streams() const noexcept
        {
            return num_streams_;
        }
    ...
    private:
        ...
        */
        ...
        uint64_t *root_operand_value_;

        uint64_t *root_quotient_value_;
        ...
        */
        ...
        mutable uint64_t *ntt_temp_ = nullptr;

        mutable size_t ntt_temp_size_ = 0;
