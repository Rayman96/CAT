...
namespace seal
{
    namespace util
    {
        ...
        __global__ void mutiply_uint_kernel(const uint64_t *operand1, const uint64_t operand2, uint64_t size, uint64_t result_uint64_count, uint64_t *result){
            ...
            uint64_t *result)
        {
            ...
            uint64_t *d_operand1, *d_result;
            checkCudaErrors(cudaMalloc((void**)&d_operand1, operand1_uint64_count * sizeof(uint64_t)));
            checkCudaErrors(cudaMalloc((void**)&d_result, result_uint64_count * sizeof(uint64_t)));
            checkCudaErrors(cudaMemcpy(d_operand1, operand1, operand1_uint64_count * sizeof(uint64_t), cudaMemcpyHostToDevice));

            mutiply_uint_kernel<<<1, 1>>>(d_operand1, 
                                        operand2,
                                        operand1_index_max, 
                                        result_uint64_count,
                                        d_result);

            checkCudaErrors(cudaMemcpy(result, d_result, result_uint64_count * sizeof(uint64_t), cudaMemcpyDeviceToHost));
            ...
            uint64_t *result)
        {
            ...
            mutiply_uint_kernel<<<1, 1>>>(operand1, 
                                        operand2,
                                        operand1_index_max, 
                                        result_uint64_count,
                                        result);
