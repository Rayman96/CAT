...
namespace seal
{
    namespace util
    {
        ...
        */
        ...
        __device__ inline uint64_t add_uint_mod_kernel(std::uint64_t operand1, std::uint64_t operand2, std::uint64_t modulus)
        {
            operand1 += operand2;
            return operand1 >= modulus ? operand1 - modulus : operand1;
        }
        ...
        */
        ...
        __device__ inline uint64_t sub_borrow_uint64_kernel(std::uint64_t operand1, std::uint64_t operand2, unsigned char borrow, unsigned long long *result) {
            auto diff = operand1 - operand2;
            *result = diff - (borrow != 0);
            return (diff > operand1) || (diff < borrow);
        }

        __device__ inline uint64_t sub_uint_mod_kernel(std::uint64_t operand1, std::uint64_t operand2, std::uint64_t modulus)
        {
            unsigned long long temp;
            std::int64_t borrow = static_cast<std::int64_t>(sub_borrow_uint64_kernel(operand1, operand2, 0, &temp));
            return static_cast<std::uint64_t>(temp) + (modulus & static_cast<std::uint64_t>(-borrow));
        }
...
// 三种不同的kernel，分别使用了不同的乘法函数
        template <typename T, typename = std::enable_if_t<is_uint64_v<T>>>
        __device__ std::uint64_t barrett_reduce_128_kernel3(const T *input, uint64_t modulus_value, uint64_t *ratio)
        {
            unsigned long long tmp1, tmp2[2], tmp3, carry;
            const std::uint64_t *const_ratio = ratio;

            // Multiply input and const_ratio
            // Round 1
            multiply_uint64_hw64_kernel(input[0], const_ratio[0], &carry);
            multiply_uint64_kernel2(input[0], const_ratio[1], tmp2);
            tmp3 = tmp2[1] + add_uint64_kernel(tmp2[0], carry, &tmp1);

            // Round 2
            multiply_uint64_kernel2(input[1], const_ratio[0], tmp2);
        ...
        __device__ std::uint64_t barrett_reduce_128_kernel2(const T *input, uint64_t modulus_value, uint64_t *ratio)
        {
            ...
            // Round 1
            multiply_uint64_hw64_kernel(input[0], const_ratio[0], &carry);
            multiply_uint64_kernel(input[0], const_ratio[1], tmp2);
            tmp3 = tmp2[1] + add_uint64_kernel(tmp2[0], carry, &tmp1);

            // Round 2
            multiply_uint64_kernel(input[1], const_ratio[0], tmp2);
            carry = tmp2[1] + add_uint64_kernel(tmp1, tmp2[0], &tmp1);
        ...
        template <typename T, typename = std::enable_if_t<is_uint64_v<T>>>
        __device__ inline std::uint64_t barrett_reduce_128_kernel(
            const T *input, uint64_t modulus_value, uint64_t *ratio)
        {
            unsigned long long tmp1, tmp2[2], tmp3, carry;
            const std::uint64_t *const_ratio = ratio;
            ...
            // Round 1
            multiply_uint64_hw64_kernel(input[0], const_ratio[0], &carry);
            multiply_uint64_hw64_kernel(input[0], const_ratio[1], tmp2);
            tmp3 = tmp2[1] + add_uint64_kernel(tmp2[0], carry, &tmp1);

            // Round 2
            multiply_uint64_hw64_kernel(input[1], const_ratio[0], tmp2);
            carry = tmp2[1] + add_uint64_kernel(tmp1, tmp2[0], &tmp1);
        ...
        */
        ...
        __device__ inline uint64_t multiply_uint_mod_kernel(uint64_t operand1, uint64_t quotiend, uint64_t operand, uint64_t modulus)
        {
            unsigned long long tmp1, tmp2;
            multiply_uint64_hw64_kernel(operand1, quotiend, &tmp1);
            tmp2 = operand * operand1 - tmp1 * modulus;
            return (tmp2 >= modulus) ? (tmp2 - modulus) : tmp2;
        }
        ...
        */
        ...
        __device__ inline std::uint64_t modulo_uint_kernel(
            const std::uint64_t *value, std::size_t value_uint64_count, 
            const uint64_t *modulu_value, uint64_t *modulu_ratio0, uint64_t *modulu_ratio1){
            if (value_uint64_count == 1)
            {
                // If value < modulus no operation is needed
                if (*value < *modulu_value)
                    return *value;
                else
                    return barrett_reduce_64_kernel(*value, *modulu_value, *modulu_ratio0);
            }
            ...
            {
                temp[0] = value[k];
                uint64_t temp_ratio[2]{*modulu_ratio0, *modulu_ratio1};
                temp[1] = barrett_reduce_128_kernel(temp, *modulu_value, temp_ratio);
            }
        ...
        */
        ...
        __device__ inline uint64_t multiply_add_uint_mod_kernel(
            std::uint64_t operand1, std::uint64_t operand2, std::uint64_t quoient2, std::uint64_t operand3,
            std::uint64_t modulus, std::uint64_t ratio, size_t i)
        {
            uint64_t temp1 = multiply_uint_mod_kernel(operand1, quoient2, operand2, modulus);
            uint64_t temp2 = barrett_reduce_64_kernel(operand3, modulus, ratio);
            uint64_t result = add_uint_mod_kernel(
                temp1,
                temp2, modulus);
        ...
        */
        ...
        SEAL_NODISCARD std::uint64_t dot_product_mod(
            const std::uint64_t *operand1, const std::uint64_t *operand2, std::size_t count, const Modulus &modulus, bool log);
