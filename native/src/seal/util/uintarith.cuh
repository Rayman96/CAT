...
namespace seal
{
    namespace util
    {
        ...
        SEAL_NODISCARD __device__ __host__ inline unsigned char add_uint64_generic(
...
// #endif
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        SEAL_NODISCARD __device__ __host__ inline unsigned char add_uint64(
            T operand1, S operand2, unsigned char carry, unsigned long long *result)
        {
        ...
        SEAL_NODISCARD __device__ __host__ inline unsigned char add_uint64(T operand1, S operand2, R *result)
        {
        ...
        template <typename T, typename S, typename R, typename = std::enable_if_t<is_uint64_v<T, S, R>>>
        __device__ inline unsigned char add_uint64_kernel(T operand1, S operand2, R *result)
        {
            *result = operand1 + operand2;
            return static_cast<unsigned char>(*result < operand1);
        }

        __device__ inline unsigned char add_uint64_carry_kernel(uint64_t operand1, uint64_t operand2, const uint64_t carry, unsigned long long *result){
            operand1 += operand2;
            *result = operand1 + carry;
            return (operand1 < operand2) || (~operand1 < carry);
        }
        ...
        __device__ inline unsigned char add_uint64_kernel (uint64_t operand1, uint64_t operand2, unsigned long long *result){
            *result = operand1 + operand2;
            return static_cast<unsigned char>(*result < operand1);
        }

        template <typename T, typename S>
        __device__ inline unsigned char add_uint128_kernel(
            const T *operand1, const S *operand2, unsigned long long *result)
        {
            unsigned char carry = add_uint64_kernel(operand1[0], operand2[0], result);
            return add_uint64_carry_kernel(operand1[1], operand2[1], carry, result + 1);
        }
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        __device__ inline void multiply_uint64_kernel2(T operand1, S operand2, unsigned long long *result128)
        {
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        __device__ inline void multiply_uint64_kernel(T operand1, S operand2, unsigned long long *result128)
        {
            auto operand1_coeff_right = operand1 & 0x00000000FFFFFFFF;
            auto operand2_coeff_right = operand2 & 0x00000000FFFFFFFF;
            operand1 >>= 32;
            ...
            T middle;
            auto left = operand1 * operand2 +
                        (static_cast<T>(add_uint64_kernel(middle1, operand2 * operand1_coeff_right, &middle)) << 32);
            auto right = operand1_coeff_right * operand2_coeff_right;
            auto temp_sum = (right >> 32) + (middle & 0x00000000FFFFFFFF);
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        __device__ inline void multiply_uint64_hw64_kernel(T operand1, S operand2, unsigned long long *hw64)
        {
            auto operand1_coeff_right = operand1 & 0x00000000FFFFFFFF;
            auto operand2_coeff_right = operand2 & 0x00000000FFFFFFFF;
            operand1 >>= 32;
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        __device__ inline void multiply_uint64_hw64_kernel2(T operand1, S operand2, unsigned long long *hw64) {
        ...
        template <std::size_t Count>
        SEAL_FORCE_INLINE void multiply_accumulate_uint64(
            const std::uint64_t *operand1, const std::uint64_t *operand2, unsigned long long *accumulator, bool log)
        {
            unsigned long long qword[2];
            multiply_uint64(*operand1, *operand2, qword);
            if (log) {
                printf("host Count: %d, oper1:%llu, oper2:%llu, qword[0] = %llu, qword[1] = %llu\n", Count, *operand1, *operand2,qword[0], qword[1]);
                
            }
            multiply_accumulate_uint64<Count - 1>(operand1 + 1, operand2 + 1, accumulator, log);
            add_uint128(qword, accumulator, accumulator);
        }
        ...
        template <std::size_t Count>
        __device__ inline void multiply_accumulate_uint64_kernel(
            const std::uint64_t *operand1, const std::uint64_t *operand2, unsigned long long *accumulator)
        {
            unsigned long long qword[2];
            multiply_uint64_kernel(*operand1, *operand2, qword);
            multiply_accumulate_uint64_kernel<Count - 1>(operand1 + 1, operand2 + 1, accumulator);
            add_uint128_kernel(qword, accumulator, accumulator);
        }

        template <>
        __device__ inline void multiply_accumulate_uint64_kernel<0>(
        ...
        SEAL_FORCE_INLINE void multiply_accumulate_uint64<0>(
            SEAL_MAYBE_UNUSED const std::uint64_t *operand1, SEAL_MAYBE_UNUSED const std::uint64_t *operand2,
            SEAL_MAYBE_UNUSED unsigned long long *accumulator, bool log)
        {
            // Base case; nothing to do
        }
        ...
        __device__ inline void divide_uint128_inplace_kernel(uint64_t *numerator, uint64_t denominator, uint64_t *result)
        {
                uint128_t n, q;
