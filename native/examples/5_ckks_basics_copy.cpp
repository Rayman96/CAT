...
void example_ckks_basics()
{
    ...
    */
    ...
    auto stage_start = chrono::high_resolution_clock::now();
    evaluator.relinearize_inplace(x3_encrypted, relin_keys);
    auto stage_end = chrono::high_resolution_clock::now();
    auto duration_relin = chrono::duration_cast<chrono::microseconds>(stage_end - stage_start);
    cout << "stage_7 duration: " << duration_relin.count() << " microseconds" << endl;
    ...
    evaluator.rotate_vector_inplace(x3_encrypted, 1, gal_keys);
...
//     x3_encrypted.scale() = pow(2.0, 60);
//     x1_encrypted.scale() = pow(2.0, 60);
...
//     evaluator.add(x3_encrypted, x1_encrypted, encrypted_result);
    // cout << "encrypted_result scale: " << log2(encrypted_result.scale()) << endl;
    // // evaluator.add_plain_inplace(encrypted_result, plain_coeff0);

    // cout << "encrypted scale: " << log2(encrypted_result.scale()) << endl;



    // evaluator.multiply_inplace(encrypted_result, x3_encrypted);
    // evaluator.relinearize_inplace(encrypted_result, relin_keys);
    // evaluator.rescale_to_next_inplace(encrypted_result);


    // evaluator.rotate_vector_inplace(encrypted_result, 1, gal_keys);

    // cout << "encrypted scale: " << log2(encrypted_result.scale()) << endl;

    // cout << "encrypted scale: " << log2(x3_encrypted.scale()) << endl;
